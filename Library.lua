...
-- ---- Addon Bindings ----
local TextManager = _G.ObsidianMatchaAddons and _G.ObsidianMatchaAddons["addons/TextManager.lua"]
assert(
    TextManager
        and TextManager.Measure
        and TextManager.Fit
        and TextManager.KeyName
        and TextManager.FormatKeybind
        and TextManager.RenderInput,
    "addons/TextManager.lua is required by Library.lua"
)
GalaxObsidian.TextManager = TextManager
local IconManager = _G.ObsidianMatchaAddons and _G.ObsidianMatchaAddons["addons/IconManager.lua"]
...
local function estimateTextWidth(text, size, font)
    return TextManager:Measure(text, size, font or Theme.Font)
end
local function fitTextToWidth(text, maxWidth, size, font)
    return TextManager:Fit(text, maxWidth, size, font or Theme.Font)
end
...
function GalaxObsidian:CreateWindow(options)
    ...
    function Window:_text(text, x, y, color, size, font, center, outline, z)
        ...
        local tx = x
        if center == true then
            tx = tx - estimateTextWidth(content, textSize, resolvedFont) / 2
        end
    ...
    function Window:_openKeybindModePopup(widget, x, y, w)
        ...
        self.KeybindModePopup = { x = x, y = y + 23, w = math.max(82, w), h = 6 + #modes * 20, z = 135 }
    ...
    function Window:_widgetHeight(widget)
        ...
        if widget.type == "label" then
            local lines = wrapTextLines(widget.text or "", 200, 14, 8, Theme.Font)
            local hasAddon = widget.addons and #widget.addons > 0
            return math.max(hasAddon and 28 or 20, #lines * 14 + 5)
        end
        if widget.type == "slider" then
            return 39
        end
        ...
        if widget.type == "textbox" then
            return 54
        end
        if widget.type == "keybox" or widget.type == "keybind" then
            return 32
        end
        return 30
    end
    ...
    function Window:_renderCheckbox(widget, x, y, w, z)
        ...
        local labelMaxW = widget.keybind and (w - keyW - 34) or (w - 26)
        local overBox = not disabled and self:_hover(x, y, w, 24, widget)
        ...
        self:_square(boxX, y + 5, 15, 15, checkBoxBg, true, disabled and 0.55 or 1, 2, z + 1)
        self:_square(boxX, y + 5, 15, 15, checkBoxOutline, false, disabled and 0.55 or 1, 2, z + 2)
        if widget.value then
            self:_drawIcon("check", boxX + 7.5, y + 12.5, 12, Theme.Text, z + 4)
        end
        self:_text(
        ...
        if keyLabel then
            ...
            self:_square(keyX, y + 2, keyW, keyH, keyBg, true, 1, 2, z + 1)
            self:_square(keyX, y + 2, keyW, keyH, keyOutline, false, 1, 2, z + 2)
            self:_text(
                fitTextToWidth(keyLabel, keyW - 10, keyTextSize, Theme.Font),
                keyX + 6,
                y + 5,
                keyText,
                keyTextSize,
    ...
    function Window:_renderToggle(widget, x, y, w, z)
        ...
        local addonSize = 18
        local addonGap = 4
        local addonAreaW = addonCount > 0 and (addonCount * addonSize + (addonCount - 1) * addonGap + 4) or 0
        local addonStartX = switchX - addonAreaW - 6
        local keyX = addonStartX - keyW - 6
        local toggleText = self:_anim(widget, "toggle.text", widget.value and Theme.Text or inactiveTextColor(), 16)
        self:_tooltip(widget, x, y, w, 28, widget)
        self:_text(
            fitTextToWidth(widget.label, w - switchW - keyW - addonAreaW - 18, 15, Theme.Font),
            x,
            y + 5,
            toggleText,
            15,
        ...
        for _, addon in ipairs(addons) do
            if addon.visible ~= false then
                ...
                self:_renderColorSwatch(addon, swatchX, y + 3, addonSize, z + 1)
                ai = ai + 1
            end
        end
        ...
        if not disabled and self:_focusClick(x, y, w, 31, widget) then
            widget.value = not widget.value
            widget._toggleAnimateUntil = tick() + 0.2
            safeCall(widget.callback, widget.value)
            safeCall(widget.changed, widget.value)
    ...
    function Window:_renderSlider(widget, x, y, w, z)
        ...
        if not compact then
            self:_tooltip(widget, x, y, w, 30, widget)
        ...
        local labelH = compact and 0 or 17
        local barX, barY, barW, barH = x, y + labelH, w, 13
        if compact then
            barY = y + 2
        end
        local percent = 0
        ...
        local centeredValueW = estimateTextWidth(valueText, 11, Theme.Font)
        ...
        if not disabled and self:_click(barX, barY - 4, barW, barH + 8) then
    ...
    function Window:_renderDropdown(widget, x, y, w, z, multi)
        ...
        self:_tooltip(widget, x, y, w, 47, widget)
        self:_text(
        ...
        self:_square(x, y + 20, w, 25, dropdownBg, true, 1, 3, z + 1)
        self:_square(x, y + 20, w, 25, dropdownOutline, false, disabled and 0.45 or 1, 3, z + 2)
        ...
        self:_drawIcon(isOpen and "chevron-up" or "chevron-down", x + w - 14, y + 33, 14, dropdownIcon, z + 3)
        widget.popup = { x = x, y = y + 44, w = w, z = 120, multi = multi }

        if not disabled and self:_focusClick(x, y + 20, w, 25, widget) then
    ...
    function Window:_renderKeybind(widget, x, y, w, z)
        ...
        local keyTextSize = 14
        local keyH = 22
        local keyW = math.max(40, math.floor(estimateTextWidth(label, keyTextSize, Theme.Font) + 26))
        local keyX = x + w - keyW
        local overKey = not disabled and self:_hover(keyX, y + 5, keyW, keyH, widget)
        ...
        self:_tooltip(widget, x, y, w, 24, widget)
        self:_text(
            fitTextToWidth(widget.label, w - keyW - 10, 14, Theme.Font),
            x,
            y + 8,
            Theme.Text,
            14,
        ...
        self:_square(keyX, y + 5, keyW, keyH, keyBg, true, 1, 2, z + 1)
        self:_square(keyX, y + 5, keyW, keyH, keyOutline, false, 1, 2, z + 2)
        self:_text(
            fitTextToWidth(label, keyW - 10, keyTextSize, Theme.Font),
            keyX + 6,
            y + 8,
        ...
        if not disabled and self.Mouse2Clicked and self:_over(keyX, y + 2, keyW, keyH) then
            self:_openKeybindModePopup(widget, keyX, y + 2, keyW)
        end
        if not disabled and self:_click(keyX, y + 2, keyW, keyH) then
            widget.listening = true
            self.KeyListenTarget = widget
    ...
    function Window:_renderTextbox(widget, x, y, w, z)
        ...
        local disabled = widget.disabled == true
        widget.hitbox = { x = x, y = y + 20, w = w, h = 25 }
        local overBox = not disabled and self:_hover(x, y + 20, w, 25, widget)
        ...
        self:_square(x, y + 20, w, 25, boxBg, true, 1, 3, z + 1)
        self:_square(x, y + 20, w, 25, boxOutline, false, disabled and 0.45 or 1, 3, z + 2)
        self:_renderTextInputValue(
            widget.value,
            widget.placeholder,
            x + 7,
        ...
        elseif self.Mouse1Clicked and not self:_over(x, y + 20, w, 25) and focused then
            self.TextTarget = nil
    ...
    function Window:_renderKeyBox(widget, x, y, w, z)
        ...
        local focused = self.TextTarget == widget
        widget.hitbox = { x = x, y = y + 2, w = boxW, h = 25 }
        self:_square(x, y + 2, boxW, 25, Theme.Surface, true, 1, 3, z + 1)
        self:_square(x, y + 2, boxW, 25, focused and self.Accent or Theme.Outline, false, 1, 3, z + 2)
        self:_renderTextInputValue(
            widget.value,
            widget.placeholder,
            x + 7,
        ...
        local bx = x + boxW + gap
        local over = self:_hover(bx, y + 2, buttonW, 25, widget)
        self:_square(bx, y + 2, buttonW, 25, over and Theme.Surface2 or Theme.Surface, true, 1, 3, z + 1)
        self:_square(bx, y + 2, buttonW, 25, over and Theme.Outline2 or Theme.Outline, false, 1, 3, z + 2)
        self:_text(
            fitTextToWidth("Execute", buttonW - 10, 13, Theme.Font),
            bx + buttonW / 2,
            y + 9,
            over and Theme.Text or Theme.Muted,
            13,
            Drawing.Fonts.Monospace,
            true,
            true,
        ...
        if self:_focusClick(x, y + 2, boxW, 25, widget) then
            self.TextTarget = widget
            self:_closeFloating("textbox")
            self:_claimInteraction(widget)
        elseif self:_click(bx, y + 2, buttonW, 25, widget) then
            safeCall(widget.executeCallback, widget.value)
            self:_releaseInteraction(widget)
        elseif self.Mouse1Clicked and focused and not self:_over(x, y + 2, boxW, 25) then
            self.TextTarget = nil
            self:_releaseInteraction(widget, true)
        end
    end
    ...
    function Window:_renderColorSwatch(widget, swatchX, swatchY, swatchSize, z)
        ...
        local popupPad = 6
        local popupTitleH = widget.title and 16 or 0
        local popupMapSize = 200
        local popupValueH = 20
        local popupH = popupPad + popupTitleH + popupMapSize + 8 + popupValueH + popupPad
        widget.popup = {
            x = swatchX - (widget.transparencyEnabled and 238 or 216),
            y = swatchY + swatchSize + 3,
            w = widget.transparencyEnabled and 256 or 234,
            h = popupH,
            z = 125,
        }
    ...
    function Window:_renderColorPicker(widget, x, y, w, z)
        ...
        local swatchY = y + 7
        self:_tooltip(widget, x, y, w, 23, widget)
        self:_text(
            fitTextToWidth(widget.label or widget.title or "ColorPicker", w - 30, 13, Theme.Font),
            x,
            y + 8,
    ...
    function Window:_renderButtonWidget(widget, x, y, w, z)
        local disabled = widget.disabled == true
        local over = not disabled and self:_hover(x, y + 2, w, 26, widget)
        ...
        self:_tooltip(widget, x, y + 2, w, 26, widget)
        self:_square(x, y + 2, w, 26, buttonBg, true, 1, 3, z + 1)
        self:_square(x, y + 2, w, 26, buttonOutline, false, 1, 3, z + 2)
        if widget._doubleConfirm and widget._confirmPending then
            ...
            if not disabled and self:_click(x, y + 2, w, 26, widget) then
            ...
            self:_text(
                fitTextToWidth(widget.label, w - 12, 13, Theme.Font),
                x + w / 2,
                y + 8,
                buttonText,
                13,
    ...
    function Window:_renderSectionTabs(widget, x, y, w, z)
        ...
        for i, tab in ipairs(widget.tabs) do
            ...
            local over = self:_hover(tx, y + 1, tw, 25, widget)
            ...
            self:_square(tx, y + 1, tw, 28, tabBg, true, 1, 3, z + 1)
            self:_text(
                fitTextToWidth(tab.name, tw - 12, 13, Theme.Font),
                tx + tw / 2,
                y + 8,
                tabText,
                13,
            ...
            if self:_click(tx, y + 1, tw, 25, widget) then
                widget.active = i
                self.Mouse1Clicked = false
            end
        end
        self:_square(x, y + 1, w, 28, Theme.Outline, false, 1, 3, z + 2)
        local active = widget.tabs[widget.active or 1]
        if not active then
            return nil
        end
    ...
    function Window:_renderWidget(widget, x, y, w, z, clipTop, clipBottom)
        ...
        if widget.type == "divider" then
            self:_line(x, y + 6, x + w, y + 6, Theme.Outline, 1, z + 2)
        elseif widget.type == "colorpicker" then
        ...
        elseif widget.type == "label" then
            self:_tooltip(widget, x, y, w, 18, widget)
            local addons = widget.addons or {}
            local addonSize = 22
            local addonGap = 4
            ...
            local yOffset = addonCount > 0 and 7 or 3
                ...
                for i, addon in ipairs(addons) do
                    if addon.visible ~= false then
                        ...
                        if addon.type == "keybind" then
                            ...
                            self:_square(ax, y + 3, aw, addonSize, Theme.Surface, true, 1, 2, z + 1)
                            self:_square(
                                ax,
                                y + 3,
                                aw,
                            ...
                            self:_text(
                                fitTextToWidth(keyLabel, aw - 10, addonTextSize, Theme.Font),
                                ax + math.floor(aw / 2),
                                y + 6,
                            ...
                            if addon.disabled ~= true and self.Mouse2Clicked and self:_over(ax, y + 3, aw, addonSize) then
                                self:_openKeybindModePopup(addon, ax, y + 3, aw)
                            end
                            if addon.disabled ~= true and self:_click(ax, y + 3, aw, addonSize) then
                            ...
                            self:_renderColorSwatch(addon, ax, y + 3, addonSize, z + 1)
                        end
                        ax = ax + aw + addonGap
                    end
                end
    ...
    function Window:_renderSections(tab, x, y, w, h, z, clipTopOverride, clipBottomOverride)
        if tab.IsKeyTab then
            ...
            if visibleCount > 1 then
                totalH = totalH + (visibleCount - 1) * 4
            end
            local wy = y
        ...
        for _, layout in ipairs(layouts) do
            if (layout.renderVisibleH or 0) > 0 then
                ...
                if section.Name and section.Name:sub(1, 2) ~= "__" and sy >= clipTop and sy + 34 <= clipBottom then
                    self:_line(sx, sy + 34, sx + layout.w, sy + 34, Theme.Outline, 1, z + 3)
                    self:_text(
                        fitTextToWidth(section.Name, layout.w - 24, 15, Theme.Font),
                        sx + 12,
                        sy + 10,
                        Theme.Text,
                        15,
                ...
                local headerH = (section.Name and section.Name:sub(1, 2) ~= "__") and 42 or 10
                local gap = 4
                local wy = sy + headerH
                for _, widget in ipairs(section.widgets) do
                    if self:_matchesSearch(widget, section) then
                        ...
                        if wy < sectionBottom and wy + wh > sectionTop then
                            self:_renderWidget(widget, sx + 10, wy, layout.w - 20, z + 5, sectionTop, sectionBottom)
                        else
                            self:_closeClippedWidget(widget)
                        end
                        wy = wy + wh + gap
    ...
    function Window:_renderDropdownPopup()
        ...
        local scrollBarW = hasScroll and 7 or 0
        local itemH = 21
        local height = visibleCount * itemH + 4
        ...
        if hasScroll then
            local trackX = info.x + info.w - scrollBarW - 2
            local trackY = info.y + 2
        ...
        local listPad = 2
        local listW = info.w - scrollBarW - (hasScroll and 2 or 0)
        local listStartY = info.y + 2
        for i = 1, visibleCount do
            ...
            self:_text(
                fitTextToWidth(option, listW - 25, 11, Theme.Font),
                info.x + 18,
                oy + 5,
                optionText,
                11,
    ...
    function Window:_renderColorPickerPopup()
        ...
        local info = widget.popup
        info.x, info.y = self:_clampToViewport(info.x, info.y, info.w, info.h, 6, true)
        local x, y, z = info.x, info.y, info.z
        local pad = 6
        local titleH = widget.title and 16 or 0
        local svX = x + pad
        local svY = y + pad + titleH
        local svSize = 200
        local hueX = svX + svSize + 6
        local barW = 16
        local alphaX = hueX + barW + 6
        local infoY = svY + svSize + 8
        ...
        self:_circle(mapX + widget.sat * mapSize, mapY + (1 - widget.vib) * mapSize, 4, Theme.Text, true, 1, z + 6)
        ...
        local boxH = 20
        local boxW = (info.w - pad * 2 - 8) / 2
        local boxTextSize = 11
        self:_square(x + pad, infoY, boxW, boxH, Theme.Main, true, 1, 3, z + 2)
        self:_square(x + pad, infoY, boxW, boxH, Theme.Outline, false, 1, 3, z + 3)
        ...
        self:_text(
            fitTextToWidth(displayHex, boxW - 12, boxTextSize, Theme.Font),
            x + pad + 5,
            infoY + 5,
            Theme.Text,
            boxTextSize,
        ...
        self:_square(x + pad + boxW + 8, infoY, boxW, boxH, Theme.Main, true, 1, 3, z + 2)
        self:_square(x + pad + boxW + 8, infoY, boxW, boxH, Theme.Outline, false, 1, 3, z + 3)
        self:_text(
            fitTextToWidth(displayRgb, boxW - 12, boxTextSize, Theme.Font),
            x + pad + boxW + 13,
            infoY + 5,
            Theme.Text,
            boxTextSize,
        ...
        if self.Mouse1Clicked and self.MouseLockOwner == widget and not self:_over(x, y - 22, info.w, info.h + 22) then
            self.ColorPickerTarget = nil
            self.ColorPickerDrag = nil
            self:_releaseInteraction(widget)
        end
    ...
    function Window:_renderKeybindMenu()
        ...
        local rowH = 24
        local width = self.KeybindMenuWidth or 260
        local dragH = 32
        local height = dragH + 28 + #rows * rowH -- Initialize persisted position
        ...
        x, y = self:_clampToViewport(x, y, width, height, 6, true) -- Drag handle area (title bar)
        local overDrag = self:_over(x, y, width, dragH) -- Drag handling
        ...
        self:_text("Keybinds", x + 10, y + 10, Theme.Text, 14, Drawing.Fonts.Monospace, false, true, 132) -- Separator line
        ...
        if #rows == 0 then
            ...
            for i, row in ipairs(rows) do
                ...
                if row.toggle then
                    ...
                    self:_square(cbX, cbY, 12, 12, checkboxBg, true, 1, 2, 132)
                    self:_square(cbX, cbY, 12, 12, checkboxOutline, false, 1, 2, 133)
                    if row.checked then
                        self:_drawIcon("check", cbX + 6, cbY + 6, 10, Theme.Text, 134)
                    end
                    if self:_click(cbX, cbY, 12, 12) then
                ...
                if
                    ...
                    and self:_over(x + 6, ry, width - 12, rowH)
                then
                    self:_openKeybindModePopup(row.widget, x + 10, ry + 2, 92)
                end
                local textColor = self:_anim(self, "keybindMenu.text." .. tostring(i), row.checked and Theme.Text or Theme.Muted, 16)
                self:_text(
                    fitTextToWidth(row.text, width - (textX - x) - 10, 13, Theme.Font),
                    textX,
                    ry + 3,
                    textColor,
                    13,
    ...
    function Window:_renderKeybindModePopup()
        ...
        for i, mode in ipairs(modes) do
            ...
            if selected then
                self:_square(x + 3, ry, w - 6, rowH - 1, Theme.Surface2, true, 1, 2, z + 1)
            end
            if selected then
                self:_drawIcon("check", x + 12, ry + math.floor(rowH / 2), 10, self.Accent, z + 3)
            end
            self:_text(mode, x + 22, ry + 4, selected and Theme.Text or Theme.Muted, 12, Drawing.Fonts.Monospace, false, true, z + 3)
            if self:_click(x + 3, ry, w - 6, rowH - 1, target) then
                target.mode = mode
    ...
    function Window:_renderTooltip()
        ...
        local w = math.floor(widestLineWidth(lines, size, Theme.Font) + pad * 2 + 26)
        local h = pad * 2 + #lines * lineH
        local x, y = self:_placeNearMouse(w, h, 12, 14, 6, true)
        self:_square(x, y, w, h, Theme.Background, true, 1, 3, 145)
        self:_square(x, y, w, h, Theme.SoftOutline, false, 1, 3, 146)
        for i, line in ipairs(lines) do
            self:_text(
                fitTextToWidth(line, w - 10, size, Drawing.Fonts.Monospace),
                x + 5,
    ...
    function Window:_render()
        ...
        local compact = w <= 420
        local sidebarW = compact and 48 or math.ceil(w * 0.26)
        if sidebarW < 128 and not compact then
            sidebarW = 128
        end
        if sidebarW > 200 and not compact then
            sidebarW = 200
        end
        local topH = 50
        local bottomH = 20
        ...
        local tabY = y + topH
        for _, tab in ipairs(self.Tabs) do
            if self:_click(x, tabY, sidebarW, 40) then
                self.ActiveTab = tab
                self:_closeFloating()
            end
            tabY = tabY + 40
        ...
        local chromeTitleIconSize = (self.IconReady and self.IconData) and math.min(self.IconSize or 26, 26) or 0
        local chromeTitleGap = chromeTitleIconSize > 0 and 6 or 0
        local chromeTitleSize = 21
        local chromeTitleText = fitTextToWidth(
            self.Title,
            sidebarW - chromeTitleIconSize - chromeTitleGap - 24,
            chromeTitleSize,
            Theme.Font
        )
        ...
        if searchVisible then
            ...
            self:_drawIcon("search", searchX + 17, searchY + searchH / 2, 17, Theme.Muted, chromeZ + 5)
            self:_renderTextInputValue(
                self.SearchText,
                self.SearchPlaceholder,
                searchX + 44,
                searchY + math.floor((searchH - 16) / 2),
                searchW - 70,
                16,
                self.SearchFocused,
                false,
        ...
        for _, tab in ipairs(self.Tabs) do
            ...
            local over = self:_hover(x, chromeTabY, sidebarW, 40, tab)
            ...
            if active then
                self:_square(x, chromeTabY, sidebarW, 40, sidebarBg, true, 1, 0, chromeZ + 2)
            end
            local iconX = x + (compact and math.floor(sidebarW / 2) or 21)
            local iconY = chromeTabY + 20
            ...
            self:_drawIcon(tab.Icon or tab.Name, iconX, iconY, 19, iconColor, chromeZ + 4)
            if not compact then
                self:_text(
                    fitTextToWidth(tab.Name, sidebarW - 42, 16, Theme.Font),
                    x + 42,
                    chromeTabY + 12,
                    tabColor,
                    16,
