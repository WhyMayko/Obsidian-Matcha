...
local function loadAddon(path)
	...
	if not chunk then
		error("Failed to load addon " .. path .. ": " .. tostring(err), 2)
	end

	local module = chunk()
	if type(module) ~= "table" and _G.ObsidianMatchaAddons then
		module = _G.ObsidianMatchaAddons[path]
	end

	if type(module) ~= "table" then
		error("Addon " .. path .. " did not return a module table. Check if the GitHub file is updated.", 2)
	end
